...
void drawFigure(){
	...
	glPushMatrix();	
		glColor3f(1,0,0);	glTranslatef(3,0,3);	glMaterialf(GL_FRONT, GL_SHININESS, 100.0);	glutSolidSphere(0.3,300,300);	
	glPopMatrix(); // RED SPHERE

	glPushMatrix();	
		glColor3f(0,0,1);	glTranslatef(3,0,2);	glMaterialf(GL_FRONT, GL_SHININESS, 100.0);glutSolidSphere(0.3,300,300);	
	glPopMatrix(); // BLUE SPHERE
	
	glPushMatrix();
		glColor3f(1,1,0);	glTranslatef(2,0,3);	glMaterialf(GL_FRONT, GL_SHININESS, 100.0);glutSolidSphere(0.3,300,300);	
	...
	//if(key['`']){ glEnable(GL_LIGHTING); }
	//else if(!key['`']){ glDisable(GL_LIGHTING); }
...
void doMotion(int X, int Y){
	...
	if(Button_left_push){
		if(leftTempY > Y){ aty+=0.1; }
		else if(leftTempY < Y){ aty-=0.1; }
		leftTempY = Y;
